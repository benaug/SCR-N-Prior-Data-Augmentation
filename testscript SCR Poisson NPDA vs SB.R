...
conf$addSampler(target = c("N"),
                type = 'zSampler',control = list(z.ups=z.ups,M=M,J=J,ind.detected=ind.detected,n=sum(ind.detected),
...
mvSamples1 <- as.matrix(Cmcmc$mvSamples)
# plot(mcmc(mvSamples1[250:nrow(mvSamples1),]))

#run Schofield and Barker version
source("NimbleModel SCR SB.R")
#constants for Nimble
constants <- list(M=M,J=J,K1D=K1D,xlim=data$xlim,ylim=data$ylim,n=data$n)

#supply data to nimble
Nimdata <- list(y=y2D,X=data$X,zerouse=0,dummy.data1=1,dummy.data2=1)

#set parameters to monitor
parameters <- c('lambda.N','p0','sigma','N') 
nt <- 1 #thinning rate
...
Rmodel <- nimbleModel(code=NimModel, constants=constants, data=Nimdata,check=FALSE,
                      inits=Niminits)
config.nodes <- c('N','lambda.N','p0','sigma') #we will add samplers for N/z and s below
conf <- configureMCMC(Rmodel,monitors=parameters, thin=nt,nodes=config.nodes) 
...
aa <- mcmc.list(mcmc(mvSamples1[250:nrow(mvSamples1),-5]),mcmc(mvSamples2[250:nrow(mvSamples2),]))
gelman.diag(aa)
plot(aa)
